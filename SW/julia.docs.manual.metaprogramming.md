---
page-title: Metaprogramming · The Julia Language
...
Metaprogramming is a powerful tool, but it introduces complexity that can make code more difficult to understand. For example, it can be surprisingly hard to get scope rules correct. Metaprogramming should typically be used only when other approaches such as [higher order functions](https://docs.julialang.org/en/v1/manual/functions/#man-anonymous-functions) and [closures](https://en.wikipedia.org/wiki/Closure_(computer_programming)) cannot be applied.
...
## [Program representation](https://docs.julialang.org/en/v1/manual/metaprogramming/#Program-representation)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Program-representation "Permalink")
...
The next step is to [parse](https://en.wikipedia.org/wiki/Parsing#Computer_languages) each string into an object called an expression, represented by the Julia type [`Expr`](https://docs.julialang.org/en/v1/base/base/#Core.Expr):
...
-   a [`Symbol`](https://docs.julialang.org/en/v1/base/base/#Core.Symbol) identifying the kind of expression. A symbol is an [interned string](https://en.wikipedia.org/wiki/String_interning) identifier (more discussion below).
...
The [`dump`](https://docs.julialang.org/en/v1/base/io-network/#Base.dump) function provides indented and annotated display of `Expr` objects:
...
### [Symbols](https://docs.julialang.org/en/v1/manual/metaprogramming/#Symbols)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Symbols "Permalink")

The `:` character has two syntactic purposes in Julia. The first form creates a [`Symbol`](https://docs.julialang.org/en/v1/base/base/#Core.Symbol), an [interned string](https://en.wikipedia.org/wiki/String_interning) used as one building-block of expressions, from valid identifiers:
...
## [Expressions and evaluation](https://docs.julialang.org/en/v1/manual/metaprogramming/#Expressions-and-evaluation)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Expressions-and-evaluation "Permalink")

### [Quoting](https://docs.julialang.org/en/v1/manual/metaprogramming/#Quoting)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Quoting "Permalink")

The second syntactic purpose of the `:` character is to create expression objects without using the explicit [`Expr`](https://docs.julialang.org/en/v1/base/base/#Core.Expr) constructor. This is referred to as *quoting*. The `:` character, followed by paired parentheses around a single statement of Julia code, produces an `Expr` object based on the enclosed code. Here is an example of the short form used to quote an arithmetic expression:
...
(to view the structure of this expression, try `ex.head` and `ex.args`, or use [`dump`](https://docs.julialang.org/en/v1/base/io-network/#Base.dump) as above or [`Meta.@dump`](https://docs.julialang.org/en/v1/base/io-network/#Base.Meta.@dump))

Note that equivalent expressions may be constructed using [`Meta.parse`](https://docs.julialang.org/en/v1/base/base/#Base.Meta.parse-Tuple{AbstractString,%20Int64}) or the direct `Expr` form:
...
### [Interpolation](https://docs.julialang.org/en/v1/manual/metaprogramming/#man-expression-interpolation)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#man-expression-interpolation "Permalink")

Direct construction of [`Expr`](https://docs.julialang.org/en/v1/base/base/#Core.Expr) objects with value arguments is powerful, but `Expr` constructors can be tedious compared to "normal" Julia syntax. As an alternative, Julia allows *interpolation* of literals or expressions into quoted expressions. Interpolation is indicated by a prefix `$`.
...
The use of `$` for expression interpolation is intentionally reminiscent of [string interpolation](https://docs.julialang.org/en/v1/manual/strings/#string-interpolation) and [command interpolation](https://docs.julialang.org/en/v1/manual/running-external-programs/#command-interpolation). Expression interpolation allows convenient, readable programmatic construction of complex Julia expressions.

### [Splatting interpolation](https://docs.julialang.org/en/v1/manual/metaprogramming/#Splatting-interpolation)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Splatting-interpolation "Permalink")
...
### [Nested quote](https://docs.julialang.org/en/v1/manual/metaprogramming/#Nested-quote)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Nested-quote "Permalink")
...
### [QuoteNode](https://docs.julialang.org/en/v1/manual/metaprogramming/#man-quote-node)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#man-quote-node "Permalink")

The usual representation of a `quote` form in an AST is an [`Expr`](https://docs.julialang.org/en/v1/base/base/#Core.Expr) with head `:quote`:
...
### [Evaluating expressions](https://docs.julialang.org/en/v1/manual/metaprogramming/#Evaluating-expressions)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Evaluating-expressions "Permalink")

Given an expression object, one can cause Julia to evaluate (execute) it at global scope using [`eval`](https://docs.julialang.org/en/v1/base/base/#Base.MainInclude.eval):
...
Every [module](https://docs.julialang.org/en/v1/manual/modules/#modules) has its own [`eval`](https://docs.julialang.org/en/v1/base/base/#Base.MainInclude.eval) function that evaluates expressions in its global scope. Expressions passed to [`eval`](https://docs.julialang.org/en/v1/base/base/#Base.MainInclude.eval) are not limited to returning values – they can also have side-effects that alter the state of the enclosing module's environment:
...
Since expressions are just `Expr` objects which can be constructed programmatically and then evaluated, it is possible to dynamically generate arbitrary code which can then be run using [`eval`](https://docs.julialang.org/en/v1/base/base/#Base.MainInclude.eval). Here is a simple example:
...
### [Functions on `Expr`essions](https://docs.julialang.org/en/v1/manual/metaprogramming/#Functions-on-Expressions)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Functions-on-Expressions "Permalink")

As hinted above, one extremely useful feature of Julia is the capability to generate and manipulate Julia code within Julia itself. We have already seen one example of a function returning [`Expr`](https://docs.julialang.org/en/v1/base/base/#Core.Expr) objects: the [`Meta.parse`](https://docs.julialang.org/en/v1/base/base/#Base.Meta.parse-Tuple{AbstractString,%20Int64}) function, which takes a string of Julia code and returns the corresponding `Expr`. A function can also take one or more `Expr` objects as arguments, and return another `Expr`. Here is a simple, motivating example:
...
## [Macros](https://docs.julialang.org/en/v1/manual/metaprogramming/#man-macros)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#man-macros "Permalink")

Macros provide a mechanism to include generated code in the final body of a program. A macro maps a tuple of arguments to a returned *expression*, and the resulting expression is compiled directly rather than requiring a runtime [`eval`](https://docs.julialang.org/en/v1/base/base/#Base.MainInclude.eval) call. Macro arguments may include expressions, literal values, and symbols.

### [Basics](https://docs.julialang.org/en/v1/manual/metaprogramming/#Basics)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Basics "Permalink")
...
We can view the quoted return expression using the function [`macroexpand`](https://docs.julialang.org/en/v1/base/base/#Base.macroexpand) (**important note:** this is an extremely useful tool for debugging macros):
...
There also exists a macro [`@macroexpand`](https://docs.julialang.org/en/v1/base/base/#Base.@macroexpand) that is perhaps a bit more convenient than the `macroexpand` function:
...
### [Hold up: why macros?](https://docs.julialang.org/en/v1/manual/metaprogramming/#Hold-up:-why-macros?)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Hold-up:-why-macros? "Permalink")

We have already seen a function `f(::Expr...) -> Expr` in a previous section. In fact, [`macroexpand`](https://docs.julialang.org/en/v1/base/base/#Base.macroexpand) is also such a function. So, why do macros exist?
...
The first call to [`println`](https://docs.julialang.org/en/v1/base/io-network/#Base.println) is executed when [`macroexpand`](https://docs.julialang.org/en/v1/base/base/#Base.macroexpand) is called. The resulting expression contains *only* the second `println`:
...
### [Macro invocation](https://docs.julialang.org/en/v1/manual/metaprogramming/#Macro-invocation)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Macro-invocation "Permalink")
...
It is important to emphasize that macros receive their arguments as expressions, literals, or symbols. One way to explore macro arguments is to call the [`show`](https://docs.julialang.org/en/v1/base/io-network/#Base.show-Tuple{IO,%20Any}) function within the macro body:
...
The argument `__source__` provides information (in the form of a `LineNumberNode` object) about the parser location of the `@` sign from the macro invocation. This allows macros to include better error diagnostic information, and is commonly used by logging, string-parser macros, and docs, for example, as well as to implement the [`@__LINE__`](https://docs.julialang.org/en/v1/base/base/#Base.@__LINE__), [`@__FILE__`](https://docs.julialang.org/en/v1/base/base/#Base.@__FILE__), and [`@__DIR__`](https://docs.julialang.org/en/v1/base/base/#Base.@__DIR__) macros.
...
### [Building an advanced macro](https://docs.julialang.org/en/v1/manual/metaprogramming/#Building-an-advanced-macro)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Building-an-advanced-macro "Permalink")

Here is a simplified definition of Julia's [`@assert`](https://docs.julialang.org/en/v1/base/base/#Base.@assert) macro:
...
That is, in the first call, the expression `:(1 == 1.0)` is spliced into the test condition slot, while the value of `string(:(1 == 1.0))` is spliced into the assertion message slot. The entire expression, thus constructed, is placed into the syntax tree where the `@assert` macro call occurs. Then at execution time, if the test expression evaluates to true, then [`nothing`](https://docs.julialang.org/en/v1/base/constants/#Core.nothing) is returned, whereas if the test is false, an error is raised indicating the asserted expression that was false. Notice that it would not be possible to write this as a function, since only the *value* of the condition is available and it would be impossible to display the expression that computed it in the error message.

The actual definition of `@assert` in Julia Base is more complicated. It allows the user to optionally specify their own error message, instead of just printing the failed expression. Just like in functions with a variable number of arguments ([Varargs Functions](https://docs.julialang.org/en/v1/manual/functions/#Varargs-Functions)), this is specified with an ellipses following the last argument:
...
There is yet another case that the actual `@assert` macro handles: what if, in addition to printing "a should equal b," we wanted to print their values? One might naively try to use string interpolation in the custom message, e.g., `@assert a==b "a ($a) should equal b ($b)!"`, but this won't work as expected with the above macro. Can you see why? Recall from [string interpolation](https://docs.julialang.org/en/v1/manual/strings/#string-interpolation) that an interpolated string is rewritten to a call to [`string`](https://docs.julialang.org/en/v1/base/strings/#Base.string). Compare:
...
Here, we want `t0`, `t1`, and `val` to be private temporary variables, and we want `time_ns` to refer to the [`time_ns`](https://docs.julialang.org/en/v1/base/base/#Base.time_ns) function in Julia Base, not to any `time_ns` variable the user might have (the same applies to `println`). Imagine the problems that could occur if the user expression `ex` also contained assignments to a variable called `t0`, or defined its own `time_ns` variable. We might get errors, or mysteriously incorrect behavior.

Julia's macro expander solves these problems in the following way. First, variables within a macro result are classified as either local or global. A variable is considered local if it is assigned to (and not declared global), declared local, or used as a function argument name. Otherwise, it is considered global. Local variables are then renamed to be unique (using the [`gensym`](https://docs.julialang.org/en/v1/base/base/#Base.gensym) function, which generates new symbols), and global variables are resolved within the macro definition environment. Therefore both of the above concerns are handled; the macro's locals will not conflict with any user variables, and `time_ns` and `println` will refer to the Julia Base definitions.
...
Getting the hygiene rules correct can be a formidable challenge. Before using a macro, you might want to consider whether a function closure would be sufficient. Another useful strategy is to defer as much work as possible to runtime. For example, many macros simply wrap their arguments in a `QuoteNode` or other similar [`Expr`](https://docs.julialang.org/en/v1/base/base/#Core.Expr). Some examples of this include `@task body` which simply returns `schedule(Task(() -> $body))`, and `@eval expr`, which simply returns `eval(QuoteNode(expr))`.
...
### [Macros and dispatch](https://docs.julialang.org/en/v1/manual/metaprogramming/#Macros-and-dispatch)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Macros-and-dispatch "Permalink")
...
## [Code Generation](https://docs.julialang.org/en/v1/manual/metaprogramming/#Code-Generation)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Code-Generation "Permalink")

When a significant amount of repetitive boilerplate code is required, it is common to generate it programmatically to avoid redundancy. In most languages, this requires an extra build step, and a separate program to generate the repetitive code. In Julia, expression interpolation and [`eval`](https://docs.julialang.org/en/v1/base/base/#Base.MainInclude.eval) allow such code generation to take place in the normal course of program execution. For example, consider the following custom type
...
The [`@eval`](https://docs.julialang.org/en/v1/base/base/#Base.@eval) macro rewrites this call to be precisely equivalent to the above longer versions. For longer blocks of generated code, the expression argument given to [`@eval`](https://docs.julialang.org/en/v1/base/base/#Base.@eval) can be a block:
...
Recall from [Strings](https://docs.julialang.org/en/v1/manual/strings/#non-standard-string-literals) that string literals prefixed by an identifier are called non-standard string literals, and can have different semantics than un-prefixed string literals. For example:

-   `r"^\s*(?:#|$)"` produces a [regular expression object](https://docs.julialang.org/en/v1/manual/strings/#man-regex-literals) rather than a string
-   `b"DATA\xff\u2200"` is a [byte array literal](https://docs.julialang.org/en/v1/manual/strings/#man-byte-array-literals) for `[68,65,84,65,255,226,136,128]`.
...
## [Generated functions](https://docs.julialang.org/en/v1/manual/metaprogramming/#Generated-functions)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Generated-functions "Permalink")

A very special macro is [`@generated`](https://docs.julialang.org/en/v1/base/base/#Base.@generated), which allows you to define so-called *generated functions*. These have the capability to generate specialized code depending on the types of their arguments with more flexibility and/or less code than what can be achieved with multiple dispatch. While macros work with expressions at parse time and cannot access the types of their inputs, a generated function gets expanded at a time when the types of the arguments are known, but the function is not yet compiled.
...
5.  Generated functions must not *mutate* or *observe* any non-constant global state (including, for example, IO, locks, non-local dictionaries, or using [`hasmethod`](https://docs.julialang.org/en/v1/base/base/#Base.hasmethod)). This means they can only read global constants, and cannot have any side effects. In other words, they must be completely pure. Due to an implementation limitation, this also means that they currently cannot define a closure or generator.
...
Note that there is no printout of [`Int64`](https://docs.julialang.org/en/v1/base/numbers/#Core.Int64). We can see that the body of the generated function was only executed once here, for the specific set of argument types, and the result was cached. After that, for this example, the expression returned from the generated function on the first invocation was re-used as the method body. However, the actual caching behavior is an implementation-defined performance optimization, so it is invalid to depend too closely on this behavior.

The number of times a generated function is generated *might* be only once, but it *might* also be more often, or appear to not happen at all. As a consequence, you should *never* write a generated function with side effects - when, and how often, the side effects occur is undefined. (This is true for macros too - and just like for macros, the use of [`eval`](https://docs.julialang.org/en/v1/base/base/#Base.MainInclude.eval) in a generated function is a sign that you're doing something the wrong way.) However, unlike macros, the runtime system cannot correctly handle a call to [`eval`](https://docs.julialang.org/en/v1/base/base/#Base.MainInclude.eval), so it is disallowed.
...
### [An advanced example](https://docs.julialang.org/en/v1/manual/metaprogramming/#An-advanced-example)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#An-advanced-example "Permalink")
...
### [Optionally-generated functions](https://docs.julialang.org/en/v1/manual/metaprogramming/#Optionally-generated-functions)[](https://docs.julialang.org/en/v1/manual/metaprogramming/#Optionally-generated-functions "Permalink")
